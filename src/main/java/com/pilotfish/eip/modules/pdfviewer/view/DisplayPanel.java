...
public class DisplayPanel extends AbstractView {
    ...
    private void init(){
        ...
        warningLabel = new JLabel("Visual artifacts may appear on image due to limitations in Java image decoding");
        warningLabel.setFont(Fonts.LABEL_FONT);
        warningLabel.setHorizontalAlignment(JLabel.CENTER);
        warningLabel.setBorder(BorderFactory.createEmptyBorder(6,3,6,3));
        ...
        displayPanel.add(warningLabel, BorderLayout.SOUTH);
    }
    ...
    public void updateView(PropertyChangeEvent evt) {
        if(evt.getPropertyName().equals(ORIENTATION_PROPERTY)){
            ...
            displayLabel.setSize(displayDocumentSize);
        }
    }
}
